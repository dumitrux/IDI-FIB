...
void MyGLWidget::initializeGL ()
{
  ...
  estat = 0;
  glm::vec3 posFocus3 = glm::vec3(0, 0, 0);
  glUniform3fv(posFocusLoc, 1, &posFocus3[0]);
  
  
  mov1 = glm::vec3(-1.5, 0, 1.5);
  posFocus1 = glm::vec3(-1.5, 1, 1.5);
  
  mov2 = glm::vec3(1.5, 0, 1.5);
  posFocus2 = glm::vec3(1.5, 1, 1.5);
...
{
  // Primer gir de 90graus per mirar direccio X+ despres 45graus per
  // mirar cap a leix Y, ja que Patr esta en la diagonal
  // GirTotal de 135graus per mirar al centre
  float gir = float(3.0*M_PI/4.0);
  
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(mov1.x+1.5, 0.0, mov1.z-1.5));
...
void MyGLWidget::modelTransformModel2 ()
{
  // Primer gir de -90graus per mirar direccio X- despres -45graus per
  // mirar cap a leix Y
  // Gir de -135graus per mirar al centre
  float gir = - float(3.0*M_PI/4.0);
  
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(mov2.x-1.5, 0.0, mov2.z-1.5));
  TG = glm::translate(TG, glm::vec3(1.5, -2.0, 1.5));
  TG = glm::rotate(TG, gir, glm::vec3(0, 1, 0));
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva)
    Proj = glm::perspective(float(M_PI/3.0), raw, radiEsc, 3.0f*radiEsc);
  else
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_F: { // canvia el tipus de focu
      estat++;
      if (estat > 2) estat = 0;
	  if(estat == 0) {
		glm::vec3 posFocus3 = glm::vec3(0, 0, 0);
    ...
    case Qt::Key_Up: {
	  if(estat == 1) {
		  if(mov1.z >= -1.25) mov1.z -= 0.25;
		  if(mov1.x <= 1.25) mov1.x += 0.25;
		  posFocus1 = glm::vec3(mov1.x, 1, mov1.z);
		  calculSCA();
	  }
	  else if (estat == 2) {
		  if(mov2.z >= -1.25) mov2.z -= 0.25;
		  if(mov2.x >= -1.25) mov2.x -= 0.25;
		  posFocus2 = glm::vec3(mov2.x, 1, mov2.z);
		  calculSCA();
	  }
      break;
    }
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  if (DoingInteractive == ROTATE)
  {
    ...
    viewTransform ();
    // Es canvia la matriu View per tant sha de recalcular
    if(estat == 1 or estat == 2) calculSCA();
  }
...
void MyGLWidget::calculSCA()
{// Pasar posicio del focus de llum en SCAs
  glm::vec3 posFocus = glm::vec3(0, 0, 0);
  if (estat == 1) posFocus = posFocus1;
  if (estat == 2) posFocus = posFocus2;
